...
var CheckCardNumber = /*#__PURE__*/function () {
  function CheckCardNumber() {
    _classCallCheck(this, CheckCardNumber);
  ...
  _createClass(CheckCardNumber, [{
    ...
    value: function assignCard() {
      ...
      var type = false;
      cardTypes.map(function (el) {
        if (el.startNum.test(cardNumber.value.substring(0, 2)) && el.numberLength.includes(cardNumber.value.length)) {
          type = true;
          return _this.checkNumber(name = el.name);
        }
      });
      if (type === false) return alert('Pdany numer nie jest zgodny z bazą dostawców');
    }
  }, {
    key: "checkNumber",
    value: function checkNumber(name) {
      var multiNums = '';
      var normalNums = [];
      var multiSum = 0;
      var normSum = 0; //Division or multiply and do not multiply the number

      this.cardNumber.value.split('').reverse().map(function (el, index) {
        return index % 2 !== 0 ? multiNums += el * 2 : normalNums.push(el);
      }); //Sum of multiplied numbers

      multiNums.split('').map(function (el) {
        return multiSum += Number(el);
      }); //Sum of not multiplied numbers

      normalNums.map(function (el) {
        return normSum += Number(el);
      });

      if ((multiSum + normSum) % 10 === 0) {
        return this.resultContainer.textContent = "Wprowadzony numer karty jest poprawny, wyda\u0142 j\u0105: ".concat(name);
      } else return this.resultContainer.textContent = "Wprowadzony numer karty jest niepoprawny!";
    }
  }, {
    key: "showInfo",
